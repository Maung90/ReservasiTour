...
{    
	...
	{
		return response()->json(Reservasi::all());
	...
	public function handleCustomReservasi($request, $reservasiId)
	{
		try{
	...
	public function calculateTotal($request)
	{
		if ($request->program_id != null) {
			...
			$dataProduk = Produk::where('id', $request->produk_id)
			->get();
			$hargaProduk = $dataProduk->sum(function ($detail) {
				return $detail->harga;
			});
